# 🌦 Real-Time Weather Dashboard using Power BI

This project showcases a real-time Weather Dashboard built using **Power BI** and live weather data fetched from a **REST API**.

---

## 🔧 Tech Stack
- Power BI
- REST API
- JSON (data format)

---

## 📈 Features
- Real-time weather metrics (temperature, humidity, etc.)
- Data fetched from live API
- Clean, interactive visualizations
- API response parsed from JSON

---

## 📂 Files
- `Weather_forecast_dashboard.pbix` – Power BI report
- `README.md` – Project details
